...
import { assets, squad } from"../data/club";
...
function PlayerPortrait({ player }) {
  if (hasPortrait(player)) {
    return (
      <div className="flex h-full w-full items-end justify-center bg-[linear-gradient(135deg,#f6f0df,#d7cfbd)]">
        <img className="h-full w-full object-contain object-bottom" src={player.image} alt={player.name} loading="lazy" decoding="async" />
      </div>
    );
  }
  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_center,rgba(242,195,25,0.2),transparent_58%),linear-gradient(135deg,rgba(18,53,31,0.56),rgba(5,4,3,0.94))]">
      ...
      <img className="relative z-10 h-28 w-28 object-contain opacity-92 drop-shadow-[0_20px_55px_rgba(0,0,0,.42)]" src={assets.logo} alt={`Escudo do Amazonas FC como imagem provisória de ${player.name}`} loading="lazy" decoding="async" />
    </div>
  );
}
...
function PlayerRow({ player, active, onSelect }) {
  return (
    <motion.button
      ...
      className={cx("relative mb-2 flex w-full items-center gap-3 overflow-hidden rounded-[8px] px-3 py-3 text-left transition last:mb-0",
              ...
              {FILTERS.map((item) => {
                ...
                return (
                  <motion.button
                    ...
                    aria-pressed={active}
                    className={cx("rounded-[8px] border px-4 py-3 text-[11px] font-extrabold uppercase tracking-[0.12em] transition",
