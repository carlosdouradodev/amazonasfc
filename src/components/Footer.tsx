import { assets, officialLinks, socialLinks } from"../data/club";
import { footerNavGroups } from"../data/siteSections";
              ...
              <div key={group.title}>
                ...
                <nav className="grid gap-2" aria-label={group.title}>
                  {group.items.map((item) => (
                    <a
                      className={`w-fit text-sm font-extrabold uppercase tracking-[0.06em] transition ${
                        isActiveRoute(route, item.path) ?"text-yellow" :"text-paper/72 hover:text-yellow"
                      }`}
                      href={item.path}
                      key={item.path}
